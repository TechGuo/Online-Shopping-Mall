          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a href="javascript:" class="mins" @click="skuNumReduce">-</a>
              </div>
              <div class="add">
...
export default {
  ...
  methods: {
    ...
    async addShopcar() {
      // 当前派发的是一个action，也向服务器发送请求，判断加入购物车是否成功
      // 下面的代码是说：调用仓库中的addOrUpdateShopCart，这个方法加上asyc，返回的一定是一个promise
      // 换言之，要么成功，要么失败
      try {
        //成功执行后
        // 先通过请求获取数据
        await this.$store.dispatch("getAddOrUpdateShopCart", {
          ...
          skuNum: this.skuNum,
        }); // 路由跳转之后需要将部分数据携带进跳转的路由页面
        // 一些简单的数据如：skuNum，通过query形式给路由组件传递过去
