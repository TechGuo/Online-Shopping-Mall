...
export default {
  ...
  mounted() {
    ...
    //进行判断，如果不是Home路由组件，将typeNav进行隐藏
  ...
  methods: {
    ...
    changeIndex: throttle(function (index) {
      ...
      //正常情况（用户慢慢的操作）：鼠标进入，每一个一级分类3，都会触发鼠标进入事件
      //非正常情况（用户操作很快）：本身全部的一级分类都应该触发鼠标进入事件，但是经过测试，只有部分3触发了/
    ...
    goSearch(event) {
      ...
      if (categoryname) {
        ...
        //判断：如果路由跳转的时候，带有params参数，也要捎带过去
